...
purge_owned_data() {
    ...
    for owned_dir in models evaluations runtime-sessions install-journal catalog-history model-archive session logs locks bin arena; do
        rm -rf "${RESOLVED_HOME:?}/$owned_dir"
    done
    for owned_file in \
        config.json runtime-profiles.json models.json link-ownership.json rules.json \
        recommend-model.json recommend-provider-facts.json model-quality.json model-quality.manifest.json calibration.json benchmark_history.json \
