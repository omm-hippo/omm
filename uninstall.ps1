...
function Remove-OmmOwnedData {
    ...
    foreach ($name in @("models", "evaluations", "runtime-sessions", "install-journal", "catalog-history", "model-archive", "session", "logs", "locks", "bin", "arena")) {
...
function Test-OmmPipxEnvironment {
    ...
    try {
        ...
        $OmmEnvironmentVerifier | & $environmentPython - $Distribution $resolvedHome $requireSource *> $null
        $ok = $LASTEXITCODE -eq 0
    } finally {
        $ErrorActionPreference = $previousErrorActionPreference
        $PSNativeCommandUseErrorActionPreference = $previousNativePref
