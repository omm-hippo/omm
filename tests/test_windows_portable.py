...
def test_windows_portable_workflow_is_pinned_and_release_gated():
    ...
    assert defender_step in workflow
    assert workflow.index(reuse_step) < workflow.index(defender_step)
    assert workflow.index(defender_step) < workflow.index(manifest_step)
    ...
    assert "Remove-MpPreference -ExclusionPath $excludedRoot" in workflow
    assert "AMServiceEnabled" in workflow
    assert "AntivirusEnabled" in workflow
    assert "Get-MpThreatDetection" in workflow
    assert "-DisableRemediation" in workflow
    assert "$scanExit -eq 2" in workflow
