...
Four global flags work either before or after the subcommand name (`omm --json search foo` and `omm search foo --json` are equivalent): `--json` (structured output, where supported — see above), `--yes`/`-y` (skip confirmation prompts), `--quiet`/`-q` (currently suppresses `benchmark`'s and `update`'s progress bars plus a background telemetry-flush notice; not yet wired into every command's other output — see [#80](https://github.com/omm-hippo/omm/issues/80)), and `--no-color` (disable ANSI colors on omm's own console output; the `NO_COLOR` environment variable does the same). Exit codes are consistent across every command: `0` success, `1` failure, `2` usage error (bad flag/argument).
